...
public class BuildForest extends Configured implements Tool {
  ...
  private int weightsEstimation; // weights estimation
  ...
  public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
    ...
    Option weightsEstimationOpt = obuilder.withLongName("weightsEstimation").withShortName("w").withRequired(true)
            .withArgument(abuilder.withName("weightsEstimation").withMinimum(1).withMaximum(1).create())
            .withDescription("Cost-Sensitive implementation").create();
    ...
    Option sizeOpt = obuilder.withLongName("size").withShortName("size").withRequired(true)
		.withArgument(abuilder.withName("size").withMinimum(1).withMaximum(1).create())
		.withDescription("Data size").create();
    ...
    Group group = gbuilder.withName("Options").withOption(dataOpt).withOption(datasetOpt).withOption(timeOpt)
        ...
        .withOption(outputOpt).withOption(helpOpt).withOption(weightsEstimationOpt).withOption(nposOpt)
        .withOption(nnegOpt).withOption(sizeOpt).create();
    ...
    try {
      ...
      weightsEstimation = Integer.parseInt(cmdLine.getValue(weightsEstimationOpt).toString());
      npos = Integer.parseInt(cmdLine.getValue(nposOpt).toString());
      nneg = Integer.parseInt(cmdLine.getValue(nnegOpt).toString());
      size = Integer.parseInt(cmdLine.getValue(sizeOpt).toString());
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug("weightsEstimation : {}", weightsEstimation);
  ...
  private void buildForest() throws IOException, ClassNotFoundException, InterruptedException {
    ...
    if (isPartial) {
      ...
      forestBuilder = new PartialBuilder(treeBuilder, dataPath, datasetPath, seed, getConf(), npos, nneg, size);
    } else {
      log.info("InMem Mapred implementation");
      forestBuilder = new InMemBuilder(treeBuilder, dataPath, datasetPath, seed, getConf(), npos, nneg, size);
    }
    ...
    if (weightsEstimation == IR) {
	    log.info("Weights Estimation: IR");
	}
	else if (weightsEstimation == OOB){
		log.info("Weights Estimation: IR");
		//log.info("Weights Estimation: OOB");
		//under construction
	}
    time = System.currentTimeMillis();
    
    DecisionForest forest = forestBuilder.build(nbTrees,weightsEstimation);
    
    time = System.currentTimeMillis() - time;
    
    if(buildTimeIsStored)
      writeToFileBuildTime(DFUtils.elapsedTime(time));
