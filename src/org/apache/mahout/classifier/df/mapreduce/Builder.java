...
public abstract class Builder {
  ...
  protected Builder(TreeBuilder treeBuilder, Path dataPath, Path datasetPath, Long seed, Configuration conf, int npos, int nneg, int size) {
  ...
  public static void setDataSize(Configuration conf, int size) {
    Preconditions.checkArgument(size > 0, "data size should be greater than 0");

    conf.setInt("mahout.rf.size", size);
  }
  ...
  public static int getDataSize(Configuration conf) {
    return conf.getInt("mahout.rf.size", -1);
  }
  ...
  private static void setWeightsEstimation(Configuration conf, int weightsEstimation){	
	conf.setInt("mahout.rf.weightsEstimation", weightsEstimation);
  }
  
  public static int getWeightsEstimation(Configuration conf) {
	return conf.getInt("mahout.rf.weightsEstimation", -1);
  }
  ...
  public DecisionForest build(int nbTrees, int weightsEstimation)
    ...
    setWeightsEstimation(conf, weightsEstimation);
    setNbNeg(conf, nneg);
	setNbPos(conf, npos);
	setDataSize(conf, size);
    
    log.info("Putting the dataset into the DistributedCache...");
