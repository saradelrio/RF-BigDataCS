...
public class DecisionTreeBuilder implements TreeBuilder {
  ...
  @Override
  public Node build(Random rng, Data data, double [] cWeights) {
	
	// compute the size of the dataset considering the class weights  
    ...
    if (data.getDataset().isNumerical(data.getDataset().getLabelId())) {
      ...
      // classification
      if (isIdentical(data)) {
        return new Leaf(data.majorityLabel(rng, cWeights));
      }
    ...
    if (attributes == null || attributes.length == 0) {
      ...
      if (data.getDataset().isNumerical(data.getDataset().getLabelId())) {
        ...
        // classification
        label = data.majorityLabel(rng, cWeights);
      }
    ...
    for (int attr : attributes) {
      Split split = igSplit.computeSplit(data, attr, cWeights);
    ...
    Node childNode;
    if (data.getDataset().isNumerical(best.getAttr())) { // NUMERICAL attribute
      boolean[] temp = null;
      ...
      Node loChild = build(rng, loSubset, cWeights);
      Node hiChild = build(rng, hiSubset, cWeights);
      ...
      for (int index = 0; index < values.length; index++) {
        ...
        children[index] = build(rng, subsets[index], cWeights);
      }
