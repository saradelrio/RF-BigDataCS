...
public class OptIgSplit extends IgSplit {
  ...
  @Override
  public Split computeSplit(Data data, int attr, double [] cWeights) {
    if (data.getDataset().isNumerical(attr)) {
      return numericalSplit(data, attr, cWeights);
    } else {
      return categoricalSplit(data, attr, cWeights);
    }
  }
   ...
   */
  private static Split categoricalSplit(Data data, int attr, double [] cWeights) {
    double[] values = data.values(attr);    
    double[][] counts = new double[values.length][data.getDataset().nblabels()];
    double[] countAll = new double[data.getDataset().nblabels()];
    ...
    for (int index = 0; index < data.size(); index++) {
      ...
      counts[ArrayUtils.indexOf(values, instance.get(attr))][(int) dataset.getLabel(instance)]+= cWeights[(int) dataset.getLabel(instance)];
      countAll[(int) dataset.getLabel(instance)]+= cWeights[(int) dataset.getLabel(instance)];
    }

    double size = 0;
    for (int index = 0; index < data.size(); index++) {
      Instance instance = data.get(index);
      size+= cWeights[(int) dataset.getLabel(instance)];
    }
  ...
  void initCounts(Data data, double[] values) {
    counts = new double[values.length][data.getDataset().nblabels()];
    countAll = new double[data.getDataset().nblabels()];
    countLess = new double[data.getDataset().nblabels()];
  }

  void computeFrequencies(Data data, int attr, double[] values, double [] cWeight) {
    Dataset dataset = data.getDataset();

    for (int index = 0; index < data.size(); index++) {
      Instance instance = data.get(index);
      counts[ArrayUtils.indexOf(values, instance.get(attr))][(int) dataset.getLabel(instance)]+= cWeight[(int) dataset.getLabel(instance)];
      countAll[(int) dataset.getLabel(instance)]+= cWeight[(int) dataset.getLabel(instance)];
    }
  }
   ...
   */
  Split numericalSplit(Data data, int attr, double [] cWeight) {
    double[] values = sortedValues(data, attr);

    initCounts(data, values);

    computeFrequencies(data, attr, values, cWeight);
    ...
    for (int index = 0; index < data.size(); index++) {
      Instance instance = data.get(index);
      size+= cWeight[(int) dataset.getLabel(instance)];
    }
   ...
   */    
  private static double entropy(double[] counts, double dataSize) {
    if (dataSize == 0) {
      return 0.0;
    }
