...
public class Data implements Cloneable {
   ...
   * @return
   */
  public int majorityLabel(Random rng, double [] cWeights) {
	// count the frequency of each label value
    double[] counts = new double[dataset.nblabels()];
    
    for (int index = 0; index < size(); index++) {
      counts[(int) dataset.getLabel(instances.get(index))]+= cWeights[(int) dataset.getLabel(instances.get(index))];
    }
	// find the label values that appears the most
	return DataUtils.maxindex(rng, counts);	
  }
  ...
  public double [] computeWeightsLabels(int weightsEstimation){
	  ...
	  double [] weights = new double [dataset.nblabels()];
	  //if (weightsEstimation == IR) {
		  // The number of occurrences of each label value		  		
		  int [] n_instances_class = new int [dataset.nblabels()]; 
		  int datasize = size();			
		  for (int i = 0; i < datasize; i++) {
		    n_instances_class[(int)dataset.getLabel(instances.get(i))]++;
		  }
