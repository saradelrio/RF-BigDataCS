...
public class Bagging {
   ...
   * Builds one tree
   */  
  public Node build(Random rng, double [] cWeights) {
    tree = buildWithLeafWeights(rng,cWeights);    
    // Compute leaf weights according to data            
    for (int index = 0; index < data.size(); index++) {
      Instance instance = data.get(index);
      tree.computeLeafMeasures(data,instance,cWeights);
    }    
    tree.computeLeafWeights();           
	return tree;
  }
  
  public Node buildWithLeafWeights(Random rng, double [] cWeights){
    log.debug("Bagging...");
    Arrays.fill(sampled, false);
	Data bag = data.bagging(rng, sampled);
		    
	log.debug("Building...");
	return treeBuilder.build(rng, bag, cWeights);
  }  
}